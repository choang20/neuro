...
actor SpatialDataEmitter {
    private var emitter_task: Task<(), Never>?
    private var ar_event_handler: AREventHandler!
    
    var stream: AsyncStream<SpatialFrameData> {
        
        return AsyncStream { continuation in
            if emitter_task == nil {
                ar_event_handler = AREventHandler(continuation: continuation)
                emitter_task = Task {
                    await ar_event_handler.begin_capture()
        //            await worker()
                }
            }
        }
...
//    func begin_capture() {
//        let ar_event_handler = AREventHandler(continuation: self.continuation!)
//        emitter_task = Task {
//            await ar_event_handler.begin_capture()
////            await worker()
//        }
//    }
    ...
    func end_capture() {
        guard let emitter_task = emitter_task else {
            return
        }
        emitter_task.cancel()
    }
...
class AREventHandler: NSObject {
    let continuation: AsyncStream<SpatialFrameData>.Continuation
    private var ar_session: ARSession!
    
    init(continuation: AsyncStream<SpatialFrameData>.Continuation) {
        self.continuation = continuation
    }
    
    func begin_capture() async {
    ...
    func session(_ session: ARSession, didUpdate frame: ARFrame){
        for anchor in frame.anchors {
            ...
            /*
             ...
             when ARKit is confident about the face's position, and false otherwise, including
             when the face is out of frame.
             */
            self.continuation.yield(.FaceDetected(
                FrameFaceData(
        ...
        self.continuation.yield(.NoFaceDetected)
    }
}
